...
class PINN_ExpAlpha_Alpha:
    ...
    def alphaFunc(self,t):
        ...
        alp = alpha
        #return alp
        bound_b = [tf.constant(0.001, dtype=tf.float32), tf.constant(1.0, dtype=tf.float32)] 
...
print("daysA:",*["%.8f"%(x) for x in T0A[0:nd]])
print("timeA:",*["%.8f"%(x) for x in T1A[0:nd]])
print("casesA:",*["%.8f"%(x) for x in I0A[0:nd]])
print("infectdA:",*["%.8f"%(x) for x in I1A[0:nd]])
print("alphaA:",*["%.8f"%(x) for x in A1A[0:nd]])
