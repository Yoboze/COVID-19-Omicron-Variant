...
df1 = pd.read_csv('C_Italy2.csv')


##########################################################################################################
# process data
today = '5/08/2022' # Update this to include more data 
days = pd.date_range(start='11/30/2021',end=today)
...
class PINN_ExpAlpha_Birational:
    ...
    def __init__(self, t, I,  layers1, X, lb, ub):
        ...
        self.beta = tf.Variable([2000.0], dtype=tf.float32)
        self.beta1 = tf.Variable([1000.0], dtype=tf.float32)
        self.kappa = tf.Variable([10.0], dtype=tf.float32)
        self.kappa1 = tf.Variable([7.0], dtype=tf.float32)
...
T_train, T_test, I_train, I_test = train_test_split(t_data, I_data, test_size = 0.3, random_state = 5)
...
corr_matrix = np.corrcoef(I0RT, I1RT[:160])
...
mapep =  mean_absolute_percentage_error(I0RT, I1RT[:160])
print("MAPE_B:", mapep)

#EV
ev = (explained_variance_score(I0RT, I1RT[:160]))
print("EV_B:",ev)

#RMSE
RMSE = np.sqrt(mean_squared_error(I0RT, I1RT[:160]))
print("RMSE_B:", RMSE)
